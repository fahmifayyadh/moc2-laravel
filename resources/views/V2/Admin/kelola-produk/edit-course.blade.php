    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
      <a href="allproduct.html"  >
        <p style="color:white;font-family: 'Rubik', sans-serif; font-weight: 600; font-size: 30px;">
          ...
          EDIT COURSE <span style="font-weight: 100;"></span>
        </p>
      </a>
    </div>
        ...
        <div class="col-12 col-md-6 col-lg-6">
          <form action="{{route('course.update',['id' => $course->id])}}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text" placeholder="Name" name="name" value="{{$course->name}}" required data-original-title="" title="" class="form-control"
            ...
            <div class="mb-3">
                <textarea class="form-control" required rows="3" placeholder="Course Detail" name="desc">{{$course->desc}}</textarea>
                @error('desc')
                <small>{{$message}}</small>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <div class="col-md-8">
                    <input class="form-control" type="file" nname="image" value="{{old('image')}}">
                </div>
                @error('image')
                <small>{{$message}}</small>
                @enderror
